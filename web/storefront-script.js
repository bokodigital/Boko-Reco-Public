...
const STOREFRONT_JS = `(function(){
  ...
  function injectRailStyles(){
    ...
    var css=".boko-rail{font-family:var(--boko-rail-bfont);margin:40px auto;max-width:1240px;padding:0 16px}"+
  ...
  function injectCartStyles(){
    ...
    var css=".boko-cart{font-family:var(--boko-cart-bfont,inherit);margin:16px 0;border-top:1px solid #e6e6e6;padding-top:14px}"+
      ".boko-cart__h{font-size:var(--boko-cart-hsize,15px);color:var(--boko-cart-hcolor,#1f1f1f);letter-spacing:1.5px;text-transform:uppercase;text-align:center;margin:0 0 10px;font-weight:500}"+
      ".boko-cart__row{display:flex;gap:10px;overflow-x:auto;padding-bottom:4px;scrollbar-width:thin}"+
      ".boko-cart__card{flex:0 0 130px;display:flex;flex-direction:column;gap:6px}"+
      ".boko-cart__card img{width:100%;height:130px;object-fit:cover;border-radius:6px;background:#f3f1ee}"+
      ".boko-cart__title{font-size:var(--boko-cart-tsize,12px);color:var(--boko-cart-tcolor,#1f1f1f);line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}"+
      ".boko-cart__price{font-size:var(--boko-cart-psize,12px);color:var(--boko-cart-pcolor,#1f1f1f);font-weight:600}"+
      ".boko-cart__add{border:none;background:var(--boko-cart-atc-bg,#1f1f1f);color:var(--boko-cart-atc-c,#fff);font:inherit;font-size:11px;letter-spacing:.5px;text-transform:uppercase;padding:7px 8px;border-radius:5px;cursor:pointer}"+
      ".boko-cart__add.is-added{background:#3f8f5f}"+
  ...
  function loadCartWidget(root,cfgCount){
    ...
    row.innerHTML="<div class='boko-cart__empty'>Loading\\u2026</div>";
    ...
    fetch(url,{headers:{Accept:"application/json"}}).then(function(r){return r.json();}).then(function(d){
      ...
      row.innerHTML="";
      if(!items.length){row.innerHTML="<div class='boko-cart__empty'>No recommendations right now.</div>";return;}
      items.forEach(function(p){row.appendChild(cartCard(p));});
    }).catch(function(){row.innerHTML="<div class='boko-cart__empty'>Couldn't load.</div>";});
  }
  function injectCart(){
    var host=findCartHost();if(!host)return;
    if(host.parentNode&&host.parentNode.querySelector("[data-boko-cart]"))return;
    injectCartStyles();
    var root=el("<div class='boko-cart' data-boko-cart><p class='boko-cart__h'>You may also like</p><div class='boko-cart__row' data-row></div></div>");
    host.parentNode.insertBefore(root,host);
    getConfig().then(function(cfg){
      var s=styleCartRoot(root,cfg);
      loadCartWidget(root,s.count);
    }).catch(function(){loadCartWidget(root,6);});
  }
  function initCart(){
    injectCart();
