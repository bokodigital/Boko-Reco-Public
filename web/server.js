...
import { mountKlaviyo } from "./boko-klaviyo.js"; // BOKO KLAVIYO
import { mountKlaviyoFeeds } from "./boko-klaviyo-feeds.js";
import { mountKlaviyoImpact, klaviyoCardHtml, klaviyoScript } from "./boko-klaviyo-card.js";
import { mountKlaviyoSync } from "./boko-klaviyo-sync.js";
import { mountKlaviyoSettings, klvSettingsCardHtml } from "./boko-klaviyo-settings.js";
import { mountKlaviyoAttrib } from "./boko-klaviyo-attrib.js";
import { mountKlaviyoProvision } from "./boko-klaviyo-provision.js";
import { mountImpact, impactCardHtml, impactScript } from "./boko-impact.js"; // BOKO IMPACT - AOV Improvement card
import { mountCr, crCardHtml, crScript } from "./boko-cr.js"; // BOKO CR - Conversion Lift card
...
const app = express();
// ---- BOKO KLAVIYO MODULE (additive; per-store email recommendations) ----
function bokoComplementaryPool(products, category, anchorId) {
  const cat = String(category || "").toLowerCase();
  return (products || []).filter(function (p) {
    return String(p.id) !== String(anchorId) && String(p.category || "").toLowerCase() !== cat && p.available !== false;
  });
}
mountKlaviyo(app, { getToken, loadProducts, recommend, complementaryPool: bokoComplementaryPool });
mountKlaviyoFeeds(app);
mountKlaviyoImpact(app, { shopFromToken: shopFromSessionToken });
mountKlaviyoSync(app, { getToken, gql });
mountKlaviyoSettings(app, { shopFromToken: shopFromSessionToken });
mountKlaviyoAttrib(app, { getToken, gql });
mountKlaviyoProvision(app);
mountImpact(app, { shopFromToken: shopFromSessionToken, getToken, tokenExchange, gql, apiSecret: API_SECRET, host: HOST, api: API }); // BOKO IMPACT
mountCr(app, { shopFromToken: shopFromSessionToken, getToken, tokenExchange, gql, apiSecret: API_SECRET, host: HOST, api: API }); // BOKO CR
...
</script><script>${impactScript()}</script><script>${crScript()}</script><script>${klaviyoScript()}</script></body></html>`;
